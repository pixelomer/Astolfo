...
#import "Astolfo_FLAnimatedImageView.h"
...
@protocol Astolfo_FLAnimatedImageViewDebugDelegate <NSObject>
@optional
- (void)debug_animatedImageView:(Astolfo_FLAnimatedImageView *)animatedImageView waitingForFrame:(NSUInteger)index duration:(NSTimeInterval)duration;
@end
#endif


@interface Astolfo_FLAnimatedImageView ()
...
#if defined(DEBUG) && DEBUG
@property (nonatomic, weak) id<Astolfo_FLAnimatedImageViewDebugDelegate> debug_delegate;
#endif

@end


@implementation Astolfo_FLAnimatedImageView
...
- (void)setAnimatedImage:(Astolfo_FLAnimatedImage *)animatedImage
{
    if (![_animatedImage isEqual:animatedImage]) {
        if (animatedImage) {
            if (super.image) {
                ...
                // See more here: https://github.com/Flipboard/Astolfo_FLAnimatedImage/issues/100
...
{
    // Presision is set to half of the `kAstolfo_FLAnimatedImageDelayTimeIntervalMinimum` in order to minimize frame dropping.
    const NSTimeInterval kGreatestCommonDivisorPrecision = 2.0 / kAstolfo_FLAnimatedImageDelayTimeIntervalMinimum;
