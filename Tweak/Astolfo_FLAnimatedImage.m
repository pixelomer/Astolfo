...
//  FLAnimatedImage.m
//  Flipboard
//  Commit 7d313ecce4b34174a5a867ba95abb9e2519adf46
//
...
// This library uses deprecated symbols for compatibility. Ignore
...
const NSTimeInterval kAstolfo_FLAnimatedImageDelayTimeIntervalMinimum = 0.02;

// An animated image's data size (dimensions * frameCount) category; its value is the max allowed memory (in MB).
// E.g.: A 100x200px GIF with 30 frames is ~2.3MB in our pixel format and would fall into the `Astolfo_FLAnimatedImageDataSizeCategoryAll` category.
typedef NS_ENUM(NSUInteger, Astolfo_FLAnimatedImageDataSizeCategory) {
    Astolfo_FLAnimatedImageDataSizeCategoryAll = 10,       // All frames permanently in memory (be nice to the CPU)
    Astolfo_FLAnimatedImageDataSizeCategoryDefault = 75,   // A frame cache of default size in memory (usually real-time performance and keeping low memory profile)
    Astolfo_FLAnimatedImageDataSizeCategoryOnDemand = 250, // Only keep one frame at the time in memory (easier on memory, slowest performance)
    Astolfo_FLAnimatedImageDataSizeCategoryUnsupported     // Even for one frame too large, computer says no.
};

typedef NS_ENUM(NSUInteger, Astolfo_FLAnimatedImageFrameCacheSize) {
    Astolfo_FLAnimatedImageFrameCacheSizeNoLimit = 0,                // 0 means no specific limit
    Astolfo_FLAnimatedImageFrameCacheSizeLowMemory = 1,              // The minimum frame cache size; this will produce frames on-demand.
    Astolfo_FLAnimatedImageFrameCacheSizeGrowAfterMemoryWarning = 2, // If we can produce the frames faster than we consume, one frame ahead will already result in a stutter-free playback.
    Astolfo_FLAnimatedImageFrameCacheSizeDefault = 5                 // Build up a comfy buffer window to cope with CPU hiccups etc.
};


#if defined(DEBUG) && DEBUG
@protocol Astolfo_FLAnimatedImageDebugDelegate <NSObject>
@optional
- (void)debug_animatedImage:(Astolfo_FLAnimatedImage *)animatedImage didUpdateCachedFrames:(NSIndexSet *)indexesOfFramesInCache;
- (void)debug_animatedImage:(Astolfo_FLAnimatedImage *)animatedImage didRequestCachedFrame:(NSUInteger)index;
- (CGFloat)debug_animatedImagePredrawingSlowdownFactor:(Astolfo_FLAnimatedImage *)animatedImage;
@end
#endif


@interface Astolfo_FLAnimatedImage ()
...
@property (nonatomic, strong, readonly) Astolfo_FLAnimatedImage *weakProxy;

#if defined(DEBUG) && DEBUG
@property (nonatomic, weak) id<Astolfo_FLAnimatedImageDebugDelegate> debug_delegate;
#endif
...
@implementation Astolfo_FLAnimatedImage
...
- (NSUInteger)frameCacheSizeCurrent
{
    ...
    if (self.frameCacheSizeMax > Astolfo_FLAnimatedImageFrameCacheSizeNoLimit) {
        frameCacheSizeCurrent = MIN(frameCacheSizeCurrent, self.frameCacheSizeMax);
    }
    
    if (self.frameCacheSizeMaxInternal > Astolfo_FLAnimatedImageFrameCacheSizeNoLimit) {
...
+ (void)initialize
{
    if (self == [Astolfo_FLAnimatedImage class]) {
        ...
        [[NSNotificationCenter defaultCenter] addObserverForName:UIApplicationDidReceiveMemoryWarningNotification object:nil queue:nil usingBlock:^(NSNotification *note) {
            ...
            // Note that Astolfo_FLAnimatedImages can be created on any thread, so the hash table must be locked.
            NSArray *images = nil;
...
- (instancetype)init
{
    Astolfo_FLAnimatedImage *animatedImage = [self initWithAnimatedGIFData:nil];
    if (!animatedImage) {
        FLLog(FLLogLevelError, @"Use `-initWithAnimatedGIFData:` and supply the animated GIF data as an argument to initialize an object of type `Astolfo_FLAnimatedImage`.");
    }
...
- (instancetype)initWithAnimatedGIFData:(NSData *)data optimalFrameCacheSize:(NSUInteger)optimalFrameCacheSize predrawingEnabled:(BOOL)isPredrawingEnabled
{
    ...
    if (self) {
            ...
            @autoreleasepool {
                ...
                if (frameImageRef) {
                    ...
                    if (frameImage) {
                        ...
                        // Support frame delays as low as `kAstolfo_FLAnimatedImageDelayTimeIntervalMinimum`, with anything below being rounded up to `kDelayTimeIntervalDefault` for legacy compatibility.
                        ...
                        if ([delayTime floatValue] < ((float)kAstolfo_FLAnimatedImageDelayTimeIntervalMinimum - FLT_EPSILON)) {
                            FLLog(FLLogLevelInfo, @"Rounding frame %zu's `delayTime` from %f up to default %f (minimum supported: %f).", i, [delayTime floatValue], kDelayTimeIntervalDefault, kAstolfo_FLAnimatedImageDelayTimeIntervalMinimum);
                            delayTime = @(kDelayTimeIntervalDefault);
                        }
        ...
        if (optimalFrameCacheSize == 0) {
            ...
            if (animatedImageDataSize <= Astolfo_FLAnimatedImageDataSizeCategoryAll) {
                _frameCacheSizeOptimal = self.frameCount;
            } else if (animatedImageDataSize <= Astolfo_FLAnimatedImageDataSizeCategoryDefault) {
                ...
                _frameCacheSizeOptimal = Astolfo_FLAnimatedImageFrameCacheSizeDefault;
            } else {
                // The predicted size exceeds the limits to build up a cache and we go into low memory mode from the beginning.
                _frameCacheSizeOptimal = Astolfo_FLAnimatedImageFrameCacheSizeLowMemory;
            }
        } else {
...
{
    Astolfo_FLAnimatedImage *animatedImage = [[Astolfo_FLAnimatedImage alloc] initWithAnimatedGIFData:data];
    return animatedImage;
}
...
- (void)addFrameIndexesToCache:(NSIndexSet *)frameIndexesToAddToCache
{
    ...
    Astolfo_FLAnimatedImage * __weak weakSelf = self;
...
+ (CGSize)sizeForImage:(id)image
{
    ...
    } else if ([image isKindOfClass:[Astolfo_FLAnimatedImage class]]) {
        Astolfo_FLAnimatedImage *animatedImage = (Astolfo_FLAnimatedImage *)image;
        imageSize = animatedImage.size;
    } else {
        // Bear trap to capture bad images; we have seen crashers cropping up on iOS 7.
        FLLog(FLLogLevelError, @"`image` isn't of expected types `UIImage` or `Astolfo_FLAnimatedImage`: %@", image);
    }
...
{
    self.frameCacheSizeMaxInternal = Astolfo_FLAnimatedImageFrameCacheSizeNoLimit;
...
- (void)didReceiveMemoryWarning:(NSNotification *)notification
{
    ...
    [NSObject cancelPreviousPerformRequestsWithTarget:self.weakProxy selector:@selector(growFrameCacheSizeAfterMemoryWarning:) object:@(Astolfo_FLAnimatedImageFrameCacheSizeGrowAfterMemoryWarning)];
    ...
    FLLog(FLLogLevelDebug, @"Attempt setting frame cache size max to %lu (previous was %lu) after memory warning #%lu for animated image: %@", (unsigned long)Astolfo_FLAnimatedImageFrameCacheSizeLowMemory, (unsigned long)self.frameCacheSizeMaxInternal, (unsigned long)self.memoryWarningCount, self);
    self.frameCacheSizeMaxInternal = Astolfo_FLAnimatedImageFrameCacheSizeLowMemory;
    ...
    if ((self.memoryWarningCount - 1) <= kGrowAttemptsMax) {
        [self.weakProxy performSelector:@selector(growFrameCacheSizeAfterMemoryWarning:) withObject:@(Astolfo_FLAnimatedImageFrameCacheSizeGrowAfterMemoryWarning) afterDelay:kGrowDelay];
    }
...
@implementation Astolfo_FLAnimatedImage (Logging)
