...
extern const NSTimeInterval kAstolfo_FLAnimatedImageDelayTimeIntervalMinimum;

//
//  An `Astolfo_FLAnimatedImage`'s job is to deliver frames in a highly performant way and works in conjunction with `Astolfo_FLAnimatedImageView`.
...
//
@interface Astolfo_FLAnimatedImage : NSObject
...
// Pass either a `UIImage` or an `Astolfo_FLAnimatedImage` and get back its size
+ (CGSize)sizeForImage:(id)image;

// On success, the initializers return an `Astolfo_FLAnimatedImage` with all fields initialized, on failure they return `nil` and an error will be logged.
...
@interface Astolfo_FLAnimatedImage (Logging)
...
#define FLLog(logLevel, format, ...) [Astolfo_FLAnimatedImage logStringFromBlock:^NSString *{ return [NSString stringWithFormat:(format), ## __VA_ARGS__]; } withLevel:(logLevel)]
